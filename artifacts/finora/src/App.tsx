    ...
    <div className="workspace" data-testid="page-dashboard">
      ...
      <div className="metric-grid">
        <article className="metric-card dark" data-testid="metric-portfolio-value">
          ...
          <span className="metric-value">$248,630</span>
          <span className="metric-detail">+$3,418 today · +11.7% YTD</span>
        </article>
...
function OnboardingPage({
  ...
  const [objective, setObjective] = useState('Grow wealth');
  const [risk, setRisk] = useState(57);
    ...
    <div className="onboarding-wrap" data-testid="page-onboarding">
      ...
      <main className="onboarding">
        <div className="progress-wrap">
          <div className="progress-meta"><span>Investor profile</span><span className="step-index">0{step} / 04</span></div>
          <div className="progress-track">{[1, 2, 3, 4].map((item) => <span className={`progress-segment${item <= step ? ' filled' : ''}`} key={item} />)}</div>
        </div>
        ...
        <div className="question">
          ...
          {step === 2 && (
            <>
              <div className="eyebrow">02 · risk budget</div>
          ...
          {step === 3 && (
            <>
              <div className="eyebrow">03 · decision lens</div>
              <h1>Choose the voice that challenges you.</h1>
              <p className="question-lead">Your suggested lens is based on the answers so far. Override it whenever your circumstances change; the agents will re-weight instantly.</p>
              ...
              <div className="persona-box" style={{ marginTop: 24 }}>
                <div className="eyebrow">Selected lens</div>
                <div className="persona-row"><div><h3>{getPersona(selectedPersona).name}</h3><p>{getPersona(selectedPersona).description}</p></div><ShieldCheck size={21} color="#8d3a3c" strokeWidth={1.5} /></div>
              </div>
            </>
          )}
          {step === 4 && (
            <>
              ...
              <h1>Your command center is calibrated.</h1>
              ...
              <div className="review-grid">
                <div className="review-row"><label>Time horizon</label><strong>{horizon}</strong></div>
                <div className="review-row"><label>Primary objective</label><strong>{objective}</strong></div>
                <div className="review-row"><label>Risk budget</label><strong>{risk}% maximum drawdown</strong></div>
        ...
        <div className="onboarding-footer">
          ...
          <button className="button-primary" type="button" data-testid={step === 4 ? 'button-enter-command-center' : 'button-onboarding-next'} onClick={next}>
            {step === 4 ? 'Enter command center' : 'Continue'} <ArrowRight size={13} />
          </button>
        </div>
      </main>
    </div>
    ...
    <RoutedErrorBoundary>
      <Switch>
        <Route path="/onboarding">
          <OnboardingPage initialPersona={personaId} onComplete={(id) => { setPersonaId(id); onLog('profile saved', `Active lens set to ${getPersona(id).name}`); setLocation('/'); }} onLog={onLog} />
        </Route>
        <Route path="/judge">
          <Shell personaId={personaId} perfOpen={perfOpen} setPerfOpen={setPerfOpen} logs={logs} onLog={onLog}>
            <JudgePage personaId={personaId} selectedSignalId={selectedSignalId} onPersonaChange={setPersonaId} onLog={onLog} onToast={onToast} />
          </Shell>
        </Route>
        <Route path="/">
          <Shell personaId={personaId} perfOpen={perfOpen} setPerfOpen={setPerfOpen} logs={logs} onLog={onLog}>
            <DashboardPage personaId={personaId} onPersonaChange={setPersonaId} onOpenJudge={(id) => { setSelectedSignalId(id); onLog('signal opened', `Reviewing ${id.toUpperCase()} in Judge Panel`); setLocation('/judge'); }} onLog={onLog} onToast={onToast} />
          </Shell>
        </Route>
    ...
    <QueryClientProvider client={queryClient}>
      <TooltipProvider>
        <WouterRouter base={import.meta.env.BASE_URL.replace(/\/$/, '')}>
          <Router personaId={personaId} setPersonaId={setPersonaId} selectedSignalId={selectedSignalId} setSelectedSignalId={setSelectedSignalId} onLog={onLog} logs={logs} perfOpen={perfOpen} setPerfOpen={setPerfOpen} onToast={onToast} />
        </WouterRouter>
        <Toaster />
